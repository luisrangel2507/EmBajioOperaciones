  ...
  return (
    <div className="relative w-full overflow-hidden rounded-xl border border-kraft-200 shadow-sm">
      <div className="relative h-40 sm:h-72">
        ...
        />
        <div className="absolute inset-0 bg-gradient-to-b from-ink-700/10 via-ink-700/20 to-ink-700/75" />
      ...
      <div className="flex flex-col gap-2 bg-ink-700 p-4 sm:absolute sm:inset-x-5 sm:bottom-4 sm:flex-row sm:gap-3 sm:bg-transparent sm:p-0">
        <HeroStat
          label="Ordenes totales"
          value={total}
          accent="border-olive-400/50"
          valueClass="text-olive-300"
        />
        <HeroStat
        ...
        >
          {urgent.length > 0 ? (
            <div className="mt-1.5 flex flex-col gap-0.5 border-l border-red-400/30 pl-2.5">
              {urgent.slice(0, 3).map((o) => (
                <span key={o.id} className="text-[11px] text-white/80">
                  {o.order_number} · {formatDate(o.due_date)}
                </span>
              ))}
            </div>
          ) : (
            <p className="mt-1 text-[11px] text-white/50">Sin vencimientos cercanos</p>
          )}
        </HeroStat>
        <HeroStat
        ...
        >
          {activeInspectors.length > 0 ? (
            ...
            <p className="mt-1 text-[11px] text-white/50">Sin inspectores activos</p>
          )}
        </HeroStat>
      </div>
    </div>
...
function HeroStat({
  ...
  return (
    <div
      className={`flex-1 rounded-lg border bg-ink-700/55 px-4 py-3 backdrop-blur-md ${accent}`}
    >
